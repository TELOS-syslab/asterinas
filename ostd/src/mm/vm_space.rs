...
impl<'b> CursorMut<'_, 'b> {
    ...
    /// Mark a range from the current slot with a status.
    ///
    /// # Panics
    ///
    ...
    pub fn mark(&mut self, len: usize, status: Status) {
        self.pt_cursor.mark(len, status);
    }
    ...
    pub fn copy_from(
        ...
        unsafe {
            self.pt_cursor
                .copy_from(&mut src.pt_cursor, len, prot_op, status_op)
        }
    }
}
...
impl TryFrom<PageTableItem> for VmItem {
    ...
    fn try_from(item: PageTableItem) -> core::result::Result<Self, Self::Error> {
        match item {
            ...
            PageTableItem::Marked { va, len, status } => Ok(VmItem::Marked { va, len, status }),
        }
    }
}

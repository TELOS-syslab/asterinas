...
impl SizeClasses {
    /// Return the `(index, SizeClassInfo)` of size class satisfying the `layout`
    /// 
    /// Return `None` for large sizes
    fn match_size_class(&self, layout: Layout) -> Option<(usize, SizeClassInfo)> {
        ...
        for (index, size_class_info) in self.0.iter().enumerate() {
            ...
            if size_class_size >= size {
                return Some((index, size_class_info.clone()));
            }
        }
    ...
    fn get_size_class_info(&self, size_class_idx: usize) -> Option<SizeClassInfo> {
        if size_class_idx < K_BASE_NUMBER_CLASSES {
            Some(self.0[size_class_idx])
        }
        else {
            None
        }
...
pub fn match_size_class(layout: Layout) -> Option<(usize, SizeClassInfo)> {
    SIZE_CLASSES.match_size_class(layout)
}

pub fn get_size_class_info(size_class_idx: usize) -> Option<SizeClassInfo> {
    SIZE_CLASSES.get_size_class_info(size_class_idx)
}
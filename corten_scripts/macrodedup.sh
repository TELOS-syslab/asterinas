...
# usage: macrodedup.sh [linux|asterinas]
...
if [ "$SYS_NAME" == "linux" ]; then
    EXTRA_MNT_CMDS="mount -t devtmpfs devtmpfs /dev; mount -t ext2 /dev/vdb /benchmark/bin/metis/data"
else
...
for THREAD_COUNT in "${THREAD_COUNTS[@]}"; do
    $BENCH_SCRIPT $SYS_NAME $BENCH_OUTPUT_FILE "$EXTRA_MNT_CMDS; /test/corten_benchdedup.sh $THREAD_COUNT"
done

...
/usr/local/qemu/bin/qemu-system-x86_64 \
    ...
    -device virtio-blk-pci,bus=pcie.0,addr=0x7,drive=x2,serial=vmetis,disable-legacy=on,disable-modern=off,queue-size=64,num-queues=1,config-wce=off,request-merging=off,write-cache=off,backend_defaults=off,discard=off,event_idx=off,indirect_desc=off,ioeventfd=off,queue_reset=off \
